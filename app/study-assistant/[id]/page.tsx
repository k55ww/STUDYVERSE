...
export default function StudyToolDetail({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const tool: any = studyTools.find((t: any) => t.id === id);

  if (!tool) {
    return (
      <main style={{ minHeight: "100vh", backgroundColor: "#030712", color: "white", padding: "50px", textAlign: "center" }}>
        <h2>Tool Not Found</h2>
        <Link href="/study-assistant" style={{ color: "#34d399" }}>Back to Study Assistant Hub</Link>
      </main>
    );
  }
  ...
  return (
    <main style={{ minHeight: "100vh", backgroundColor: "#030712", color: "white", padding: "50px 20px", display: "flex", justifyContent: "center" }}>
      <div style={{ maxWidth: "600px", width: "100%", background: "#1e293b", padding: "30px", borderRadius: "20px", border: "1px solid #334155" }}>
        <Link href="/study-assistant" style={{ color: "#34d399", textDecoration: "none", fontSize: "14px", fontWeight: "600" }}>← Back to Study Hub</Link>
        ...
        <h3 style={{ fontSize: "16px", marginBottom: "10px" }}>Key Features:</h3>
        <ul style={{ color: "#94a3b8", fontSize: "14px", marginBottom: "20px", paddingLeft: "20px" }}>
