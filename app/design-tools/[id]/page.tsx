...
export default function DesignToolDetail({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const tool: any = designTools.find((t: any) => t.id === id);

  if (!tool) {
    return (
      <main style={{ minHeight: "100vh", backgroundColor: "#030712", color: "white", padding: "50px", textAlign: "center" }}>
        <h2>Tool Not Found</h2>
        <Link href="/design-tools" style={{ color: "#34d399" }}>Back to Design Hub</Link>
      </main>
    );
  }
  ...
  return (
    <main style={{ minHeight: "100vh", backgroundColor: "#030712", color: "white", padding: "50px 20px", display: "flex", justifyContent: "center" }}>
      <div style={{ maxWidth: "600px", width: "100%", background: "#1e293b", padding: "30px", borderRadius: "20px", border: "1px solid #334155" }}>
        <Link href="/design-tools" style={{ color: "#34d399", textDecoration: "none", fontSize: "14px", fontWeight: "600" }}>← Back to Design Hub</Link>
